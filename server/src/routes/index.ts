...
api.get("/dashboard/summary", (_req, res) => {
  ...
  res.json({
    data: {
      ...
      inProgress: state.cases.filter((c) => c.status === "In Progress").length,
      completed: state.cases.filter((c) => c.status === "Completed").length,
      delayed: state.tasks.filter((t) => t.status === "OVERDUE").length,
      atRisk: state.cases.filter((c) => ["High", "Critical"].includes(c.risk))
        .length,
      pendingApprovals: state.tasks.filter((t) => t.status === "PENDING")
        .length,
...
api.post(
  ...
  (req, res) => {
    ...
    const project = {
      id: `project-${Date.now()}`,
      projectId: input.projectId || `NLA-${Date.now()}`,
      ...input,
      authority: input.authority || "Demo Authority",
      tehsil: input.tehsil || "Demo Tehsil",
      village: input.village || "Demo Village",
      status: "Draft",
      progress: 0,
      targetDate: input.targetDate || "2027-12-31",
      description: input.description || "Demo project",
      alignment: (input.alignment || []) as [number, number][],
      workflowTemplateId: "nh-act-1956",
    ...
    notify(state, {
      ...
      message: `${project.name} is ready for parcel association.`,
      severity: "INFO",
...
api.get("/cases", (_req, res) => {
  ...
  res.json({
    data: state.cases.map((c) => ({
      ...c,
      projectName: state.projects.find((p) => p.id === c.projectId)?.name || "",
      parcelId:
        state.parcels.find((p) => p.id === c.parcelId)?.externalId || "",
      surveyNumber:
        state.parcels.find((p) => p.id === c.parcelId)?.surveyNumber || "",
      village: state.parcels.find((p) => p.id === c.parcelId)?.village || "",
      state: "Punjab",
      district: "Mohali",
      stage: c.currentStage,
      officer:
        state.users.find((u) => u.id === c.assignedOfficerId)?.displayName ||
        "Unassigned",
    })),
...
api.get("/cases/:id", (req, res) => {
  ...
  return item
    ? res.json({
        data: {
          ...item,
          project: state.projects.find((p) => p.id === item.projectId),
          parcel: state.parcels.find((p) => p.id === item.parcelId),
          tasks: state.tasks.filter((t) => t.caseId === item.id),
          compensation: state.compensations.find((c) => c.caseId === item.id),
          possession: state.possessions.find((p) => p.caseId === item.id),
          rr: state.rr.find((r) => r.caseId === item.id),
        },
      })
    : res
        .status(404)
...
api.post(
  "/parcels",
  roles("SUPER_ADMIN", "NATIONAL_ADMIN", "PROJECT_OFFICER", "FIELD_OFFICER"),
  (req, res) => {
    const input = z
      .object({
        projectId: z.string(),
        ...
        village: z.string(),
        totalArea: z.number(),
        requiredArea: z.number(),
        geometry: z.array(z.array(z.number())).min(3),
      })
      .parse(req.body);
    const state = loadState();
    const parcel = {
      id: uid(),
      ...input,
      geometry: input.geometry as [number, number][],
      state: "Punjab",
      district: "Mohali",
      tehsil: "Kharar",
      ownerReference: `DEMO-OWNER-${Date.now()}`,
      sourceSystem: "N-LAMS demo / simulated",
      sourceReference: input.externalId,
      acquisitionStatus: "PROPOSED",
    };
    state.parcels.push(parcel);
    audit(state, req.auth!.userId, "PARCEL_CREATED", "PARCEL", parcel.id, {
      source: "simulated",
    });
    saveState(state);
...
api.post(
  "/cases",
  roles("SUPER_ADMIN", "NATIONAL_ADMIN", "PROJECT_OFFICER", "DISTRICT_OFFICER"),
  (req, res) => {
    const input = z
      .object({
        projectId: z.string(),
      ...
      .parse(req.body);
    const state = loadState();
    const parcel = state.parcels.find((p) => p.id === input.parcelId);
    if (!parcel)
    ...
    const item = {
      ...
      progress: 0,
      externalRefs: [
        { system: "Mock Land Records", id: parcel.sourceReference },
      ],
      createdAt: new Date().toISOString(),
    };
    state.cases.push(item);
    parcel.caseId = item.id;
    parcel.acquisitionStatus = "CASE_CREATED";
    audit(state, req.auth!.userId, "CASE_CREATED", "CASE", item.id, {});
    saveState(state);
...
api.get("/cases/:id/timeline", (req, res) => {
  const state = loadState();
  res.json({ data: state.audits.filter((a) => a.entityId === req.params.id) });
});
api.get("/cases/:id/workflow", (req, res) => {
  const state = loadState();
  const item = state.cases.find((c) => c.id === req.params.id);
  res.json({
    data: item
      ? state.tasks
          ...
          .map((task) => ({
            ...task,
            stage: state.stages.find((s) => s.id === task.stageId),
          }))
      : [],
  });
});
api.get("/gis/projects/:id/parcels", (req, res) => {
  const state = loadState();
  const parcels = state.parcels.filter((p) => p.projectId === req.params.id);
  res.json({
    data: {
      ...
      features: parcels.map((p) => ({
        ...
        properties: { ...p, demo: true, label: "Demo / Simulated Parcel Data" },
      })),
    },
  });
});
api.get("/notifications", (req, res) => {
  ...
  res.json({
    data: state.notifications.filter(
      (n) => !n.recipientId || n.recipientId === req.auth?.userId,
    ),
  });
});
...
api.get(
  "/audit",
  roles("SUPER_ADMIN", "NATIONAL_ADMIN", "DEPARTMENT_ADMIN", "REVIEWER"),
...
api.get("/reports/summary", (_req, res) => {
  ...
  res.json({
    data: state.projects.map((p) => ({
      ...
      progress: p.progress,
      compensationAssessed: state.compensations
        .filter(
          (c) => state.cases.find((x) => x.id === c.caseId)?.projectId === p.id,
        )
        .reduce((s, c) => s + c.assessedAmount, 0),
      compensationPaid: state.compensations
        .filter(
          (c) => state.cases.find((x) => x.id === c.caseId)?.projectId === p.id,
        )
...
api.get("/compensation", (_req, res) =>
  res.json({ data: loadState().compensations }),
);
api.patch(
  "/compensation/:id",
  roles("SUPER_ADMIN", "NATIONAL_ADMIN", "DEPARTMENT_ADMIN", "PROJECT_OFFICER"),
...
api.post(
  ...
  async (req, res) => {
    ...
    const result = await new MockPfmsAdapter().getPaymentStatus(item.caseId);
    item.paidAmount = result.paidAmount;
    item.paymentReference = result.reference;
    item.status = result.status === "PAID" ? "PAID" : "PARTIALLY_PAID";
    ...
    state.syncLogs.unshift({
      ...
      status: "MOCK",
      records: 1,
      message: "Simulated PFMS synchronization",
...
api.get("/possession", (_req, res) =>
  res.json({ data: loadState().possessions }),
);
api.patch(
  "/possession/:id",
  ...
  (req, res) => {
    ...
    const input = z
      .object({ status: z.string(), remarks: z.string().optional() })
      .parse(req.body);
    Object.assign(item, input, {
      officerId: req.auth!.userId,
      possessionDate:
        input.status === "POSSESSION_COMPLETED"
...
api.post(
  ...
  (req, res) => {
    ...
    const stage = state.stages.find(
      (s) => s.templateId === "nh-act-1956" && s.sequence === 1,
    )!;
    const task = state.tasks.find((t) => t.caseId === item.id) || {
      id: uid(),
      caseId: item.id,
      stageId: stage.id,
      status: "PENDING" as const,
      dueAt: new Date(Date.now() + stage.slaDays * 86400000).toISOString(),
    };
    task.status = "IN_PROGRESS";
    task.assignedUserId =
      task.assignedUserId ||
      state.users.find((u) => u.role === stage.responsibleRole)?.id;
    if (!state.tasks.some((t) => t.id === task.id)) state.tasks.push(task);
    item.currentStage = stage.name;
    item.status = "In Progress";
    item.progress = 10;
    audit(state, req.auth!.userId, "WORKFLOW_STARTED", "CASE", item.id, {
      legalSection: stage.legalSection,
    });
    notify(state, {
  ...
  (req, res) => {
    ...
    if (next) {
      const nextTask = {
        ...
        stageId: next.id,
        status: "PENDING" as const,
        assignedUserId: state.users.find((u) => u.role === next.responsibleRole)
          ?.id,
      ...
      item.progress = Math.round(
        ((next.sequence - 1) / state.stages.length) * 100,
      );
      notify(state, {
        recipientId: nextTask.assignedUserId,
...
api.post(
  ...
  (req, res) => {
    ...
    task.remarks = req.body.remarks || "";
    audit(state, req.auth!.userId, "TASK_REJECTED", "TASK", task.id, {});
    saveState(state);
    res.json({ data: task });
  },
);
...
api.get("/integrations", (_req, res) => {
  const state = loadState();
  res.json({
    data: ["land-records", "acquisition", "pfms", "gis"].map((system) => ({
      system,
      adapter:
        system === "pfms"
          ? "MockPfmsAdapter"
      ...
      status: "MOCK",
      lastSyncedAt:
        state.syncLogs.find((s) => s.system === system)?.syncedAt || null,
      records: state.syncLogs.find((s) => s.system === system)?.records || 0,
      label: "DEMO / MOCK",
    })),
  });
});
api.post(
  ...
  async (req, res) => {
    ...
    const result =
      system === "pfms"
        ? await new MockPfmsAdapter().getPaymentStatus("DEMO-SYNC")
        : system === "land-records"
          ? await new MockLandRecordsAdapter().getParcel("DEMO-SYNC")
          : { status: "OK" };
    const recordCount =
      system === "land-records"
    ...
    const log = {
      ...
      status: "MOCK",
      records: recordCount,
      message: "Simulated adapter synchronization completed",
      syncedAt: new Date().toISOString(),
    };
